...
data "template_file" "mongodb_node2" {
  ...
  vars = {
    node_hostname             = "${var.node2_hostname}"
...
resource "aws_instance" "sage_test_mongodb_node2" {
  ami           = "ami-dc8d97a5"
  instance_type = "t2.micro"

  tags = {
    Name        = "sage_${var.environment}_mongodb_node2"
  ...
  user_data  = "${data.template_file.mongodb_node2.rendered}"
